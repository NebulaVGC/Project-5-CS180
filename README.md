...
Student.java:
----------------------------------
The Student class requires only the students username as a parameter so that it can later be used when writing the students file to pass back to the teacher for grading. The student class should be called after the login so that this parameter can be passed to it. The student class begins by prompting the user to input the username of their teacher via a drop down menu that has all the teachers, thus requiring no input validation. The teacher names are received from the server. If no teachers are found, it will print a GUI message saying that no teachers exist. If it does exist then next the user will be prompted to choose a course out of the courses that the teacher has previously created, this is done via a drop down menu as well. These courses are stored in a file named “teachersUsername_Courses.txt”. Once a course is selected the student will be prompted to choose a quiz that the teacher has created via a drop down menu once again. These quizzes are stored in a file called “teachersUsername_courseName.txt”. Once a student selects a quiz the student class will check whether or not the student has taken the quiz before. This is done by checking for whether or not a file already exists called “studentUsername_teacherUsername_courseName_quizName.txt”. This is the file that is created when a student completes a quiz. If the quiz has already been taken then the student will be prompted between returning to the course selection or viewing the grade on their quiz as a GUI. If they choose to view their quiz then if the teacher has graded the quiz it will display the percent given by the teacher and if it has not been graded it will display that the quiz has not yet been graded. If the quiz has not been taken yet then the selection quiz(either active or imported file) will be an option GUI between the two. This is done by reading the quiz file and displaying each question one at a time with a drop down menu for options A,B,C and D. The student will then have the option to import a file with answers or to answer manually. When the quiz is finished a file will be created called “studentUsername_teacherUsername_courseName_quizName.txt” which will store a copy of the quiz taken, the students username, the students answers, and a timestamp of when they finished. After the quiz is completed, it directs the student back to the opening page which displays the teacher drop down menu. Hitting the red x at any points stops the program. 
